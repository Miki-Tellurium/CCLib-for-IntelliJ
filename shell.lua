...
--- The [shell] API provides access to CraftOS's command line interface. It
--- allows you to start programs, add completion for a program, and much more.
...
--- Run a program with the supplied arguments. Unlike [shell.run], each argument is
--- passed to the program verbatim. While [shell.run] ("echo", "b c") runs echo with
--- b and c, [shell.execute] ("echo", "b c") runs echo with a single argument b c.
...
--- Run a program with the supplied arguments. All arguments are concatenated
--- together and then parsed as a command line. As a result, [shell.run] ("program a b")
--- is the same as [shell.run] ("program", "a", "b").
---@param ... string The program to run and its arguments
---@return boolean Whether the program exited successfully
function shell.run(...) end

--- Exit the current shell. This does not terminate your program, it simply makes the
--- shell terminate after your program has finished. If this is the toplevel shell,
--- then the computer will be shutdown.
function shell.exit() end

--- Return the current working directory. This is what is displayed before the > of the
--- shell prompt, and is used by [shell.resolve] to handle relative paths.
---@return string The current working directory
function shell.dir() end

--- Set the current working directory. Throws if the path does not exist or is not a directory.
---@param dir string The new working directory
function shell.setDir(dir) end

--- Set the path where programs are located. The path is composed of a list of directory
--- names in a string, each separated by a colon (:). On normal turtles will look in the
--- current directory (.), /rom/programs and /rom/programs/turtle folder, making the path
--- .:/rom/programs:/rom/programs/turtle.
---@return string The current shell's path
function shell.path() end

--- Set the current program path. Be careful to prefix directories with a /. Otherwise they
--- will be searched for from the current directory, rather than the computer's root.
---@param path string The new program path
function shell.setPath(path) end

--- Resolve a relative path to an absolute path. The [fs] and [io] APIs work using absolute paths,
--- and so we must convert any paths relative to the current directory to absolute ones. This
...
---@param include_hidden boolean | nil Include hidden files, namely, any which start with .
---@return table A list of available programs
function shell.programs(include_hidden) end

--- Complete a shell command line. This accepts an incomplete command, and completes the program
--- name or arguments. For instance, l will be completed to ls, and ls ro will be completed to ls rom/.
...
--- * The current shell. As completion functions are inherited, this is not guaranteed to be the
--- shell you registered this function in;
...
--- should be completed to pastebin put rom/startup.lua. You completion entries may also be followed
--- by a space, if you wish to indicate another argument is expected.
---@param program string The path to the program, this should be an absolute path without the leading /.
---@param complete function | nil The completion function
function shell.setCompletionFunction(program, complete) end

--- Get a table containing all completion functions. This should only be needed when building custom
--- shells. Use [shell.setCompletionFunction] to add a completion function.
...
--- Get the current aliases for this shell. Aliases are used to allow multiple commands to refer
--- to a single program. For instance, the list program is aliased dir or ls. Running ls, dir or
--- list in the shell will all run the list program.
---@return table A table, where the keys are the names of aliases, and the values are the path to the program
function shell.aliases() end

--- Open a new [multishell] tab running a command. This behaves similarly to [shell.run], but instead
--- returns the process index. This function is only available if the [multishell] API is.
